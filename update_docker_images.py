...
#    response.raise_for_status()
    ...
    print data
    response = requests.post('%s/%s/build/' % (endpoint, repo), headers=headers, json=data)
    response.raise_for_status()
...
try:
    ...
    #build('appuio/oso-centos7-ops-base', 'v1.0.0', 'ansible-role-openshift-zabbix-monitoring', '/vendor/openshift-tools/docker/oso-ops-base/centos7/')
    #build('appuio/oso-centos7-zabbix-server', 'v1.0.0', repo, '/vendor/openshift-tools/docker/oso-zabbix-server/centos7/')
    #build('appuio/oso-centos7-zabbix-web', 'v1.0.0', repo, '/vendor/openshift-tools/docker/oso-zabbix-web/centos7/')
    #build('appuio/oso-centos7-zagg-web', 'v1.0.0', repo, '/vendor/openshift-tools/docker/oso-zagg-web/centos7/')
    #build('appuio/oso-centos7-host-monitoring', 'v1.0.0', github_project, '/vendor/openshift-tools/docker/oso-host-monitoring/centos7/')
    i = 1
    while os.environ.get('BUILD_IMAGE%d' % i, None):
        for build_tag in build_tags:
            ...
            print "%s, %s, %s, %s" % (docker_repo, build_tag, github_project, docker_context)
        i += 1
